...
#define Mouse_Speed 800.0f
#define go_Speed	300.0f									
#define turn_Speed  170.0f
...
void CalculateFrameRate()
{
    ...
    if( currentTime - lastTime > 1.0f )
    {
		...
		// 打印标题
		sprintf(strFrameRate, "三维地形模拟示意----Jackie Lee");
...
//	static float currentRotX = 0.0f;
	...
	// 计算角度
	angleY = (float)( (middleX - mousePos.x) ) / Mouse_Speed;		
	angleZ = (float)( (middleY - mousePos.y) ) / Mouse_Speed;		
...
{	
	// 获得当前帧率
	float speed_g = go_Speed * g_FrameInterval;
	float speed_t = turn_Speed*g_FrameInterval;
	// 是否按下UP箭头键或'W'键
